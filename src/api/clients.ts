...
export const getClients = async ({ page = 1, sortBy, sortOrder }: GetClientsQuery) => {
	...
	const { data } = await dbInstance.get<{ results: ClientDTO[]; total: number }>(`/clients`, {
		params,
	});
	return data;
};
...
export const createClient = async (clientValues: ClientValues) => {
	const { data } = await dbInstance.post("/clients", {
		...clientValues,
	});
	return data;
};
...
export const editClient = async (clientId: string, clientValues: ClientValues) => {
	const { data } = await dbInstance.put(`/clients/${clientId}`, clientValues);
	return data;
};
